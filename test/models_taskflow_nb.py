...
def _():
    ...
    # Ensure repo root is on path when run by marimo (cells run from a temp dir)
    for _root in [Path.cwd(), Path.cwd().parent]:
        if (_root / "baseline").is_dir():
    ...
    from baseline.models.Backbone.backbone import make_backbone
    from baseline.models.Head.head import make_head
